...
def main():
    ...
    # -------------------------------
    # 1️⃣ Segmentation Generator
    # -------------------------------
    # For segmentation: usually semantic_nc + 8 (pose + cloth)
    seg = SegGenerator(opt, input_nc=opt.semantic_nc + 8, output_nc=opt.semantic_nc)

    # -------------------------------
    # 2️⃣ GMM (Geometric Matching Module)
    # -------------------------------
    # Use 7 input channels for inputA (as per checkpoint), and 3 for inputB
    gmm = GMM(opt, inputA_nc=7, inputB_nc=3)

    # -------------------------------
    # 3️⃣ ALIAS Generator
    # -------------------------------
    # Temporarily store original semantic_nc to restore after setting
    original_nc = opt.semantic_nc
    opt.semantic_nc = 7  # for consistent alias input setup
    alias = ALIASGenerator(opt, input_nc=9)
    opt.semantic_nc = original_nc  # restore to 13 after initialization

    # -------------------------------
    # 4️⃣ Load checkpoints safely
    # -------------------------------
    print("\nLoading checkpoints...")
    ...
    # -------------------------------
    # 5️⃣ Move to GPU and eval mode
    # -------------------------------
    seg = seg.cuda().eval()
    gmm = gmm.cuda().eval()
    alias = alias.cuda().eval()
